//
//  AttractorsCalculator+CPU.swift
//  MF Metal Example
//
...
class AttractorsCalculator: WorldCalculator {
      
    init(world: WorldBuffers) throws {
        try super.init(world: world, functionName: "computeAttractorsLocations")
    }
    ...
    override func encodeCommand(commandBuffer: MTLCommandBuffer, rpd: MTLRenderPassDescriptor? = nil) throws {
        ...
        let nAttractors = world.numberOfAttractors
        guard nAttractors > 0 else {
            throw Errors.attractorsBufferEmpty
        }
        ...
        let particlesPerGroup = Int(ceil(Double(nAttractors) / Double(threadsPerThreadgroupSize.width)))
        let numberOfParticleGroups = Int(ceil(Double(nAttractors) / Double(particlesPerGroup)))
        ...
        // Pass buffers to GPU
        computeEncoder.setBuffer(world.attractorsBuffer, offset: 0, index: 0)
        computeEncoder.setBuffer(world.groupsBuffer, offset: 0, index: 1)
